...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-50">
      ...
      {/* Mobile Navigation */}
      {menuOpen && (
        <nav className="md:hidden border-t border-gray-100 bg-white/95 backdrop-blur-sm shadow-lg">
          <div className="container mx-auto px-4 py-3 space-y-1">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:text-white hover:bg-gradient-to-r hover:from-primary hover:to-accent rounded-lg transition-all duration-200"
